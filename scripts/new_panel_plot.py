...
plt.pcolormesh(yedges,xedges, np.transpose(np.ma.masked_invalid(residue[0])),cmap=plt.cm.jet_r)
plt.colorbar()
plt.title("MAGPHYS/UV")
plt.show()




min1 = np.min(np.ma.masked_invalid(env[0]))
max1 = np.max(np.ma.masked_invalid(env[0]))
min2 = np.min(np.ma.masked_invalid(ms[0]))
max2 = np.max(np.ma.masked_invalid(ms[0]))
min3 = np.min(np.ma.masked_invalid(uvs[0]))
max3 = np.max(np.ma.masked_invalid(uvs[0]))
...
cs1 = ax1.pcolormesh(yedges,xedges,(np.transpose(number)),cmap = plt.cm.jet_r)
...
cs2 = ax2.pcolormesh(yedges,xedges,np.ma.masked_invalid(np.transpose(np.log10(env[0]))), norm = norm, cmap=plt.cm.jet)
...
bounds = np.linspace(-14, -8, 21)
norm = colors.BoundaryNorm(boundaries=bounds, ncolors=256)

cs3 = ax3.pcolormesh(yedges,xedges, np.transpose(np.ma.masked_invalid(ms[0])),norm = norm,cmap=plt.cm.jet_r)
...
cs4 = ax4.pcolormesh(yedges,xedges, np.transpose(np.ma.masked_invalid(uvs[0])),norm = norm, cmap=plt.cm.jet_r)
...
f.text(0.89, 0.75, 'Environment', ha='center', va='center', rotation=270, fontsize = 14)
f.text(0.09, 0.25, 'MAGPHYS', ha='center', va='center', rotation=90, fontsize = 14)
f.text(0.89, 0.25, 'UV', ha='center', va='center', rotation=270, fontsize = 14)
